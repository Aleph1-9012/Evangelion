...
Fixed 2560×1440 canvas, centered on larger display modes.

Install with the shared `install.sh` or `sudo eva` chooser. Four horizontal cards display real GRUB entries, with wrapped titles and live numbering. Very long titles end with an ellipsis when the card is full. Up and Down move selection; larger menus scroll. The countdown uses the actual timeout and disappears completely when cancelled.
